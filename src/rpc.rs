...
/// parse errors and handler failures are surfaced as JSON-RPC error responses.
pub fn handle_line(line: &str) -> Value {
    ...
    match method.as_str() {
        ...
        // DDL generation (pure — no connection params sent by the host).
...
/// Maps a handler `Result` into a JSON-RPC response.
